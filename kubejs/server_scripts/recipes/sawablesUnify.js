...
function create_cutting(event, variant) {
    ...
    // mod blacklist
    if (modID == 'minecraft' || variant.modId == 'autumnity' || variant.modId == 'upgrade_aquatic') {
        return;
    }
...
function immersiveengineering_sawing(event, variant, sawDust) {
    ...
    if (Item.exists(variant.logBlockStripped)) {
        event.recipes.immersiveengineering
            .sawmill(
                Item.of(variant.plankBlock, 6),
                [variant.logBlockStripped, variant.woodBlockStripped],
                [
                    {
                        stripping: false,
                        output: sawDust
...
function thermal_sawing(event, variant, sawDust) {
    // mod blacklist
    if (variant.modId == 'minecraft' || variant.modId == 'autumnity' || variant.modId == 'upgrade_aquatic') {
        return;
    }
